...
function getModeForDate(dateId) {
  ...
  console.log(
    `🗓️  ${dateId} (${wd}) → Modus: ${mode}${
      envMode ? " (ENV)" : " (Fallback CLI)"
    }`
  );
  return mode;
}
 ...
 */
async function processDayBooking(
  untenFrame,
  page,
  dateId,
  modeForDay /*Remote|Office*/
) {
  const row = await untenFrame.$(
    `tr.grid_row.grid_row_pr_${cssEscape(dateId)}`
  );
  ...
  if (!projectClickable)
    throw new Error("Projektfeld im Formular nicht gefunden");
  ...
  await delay(20000); // kurze Atempause

  // Speichern & schließen
  await saveAndCloseForm(formFrame, untenFrame);
  console.log(
    `✅ Gebucht @ ${dateId} ${KOMMEN_TIME}–${GEHEN_TIME} (${modeForDay})`
  );
}
...
async function setTimeInForm(formFrame) {
  ...
  const fromCandidates = [
    '[id="1173_from"]',
    '[name="1173[from]"]',
    "#row_ZEIT input.stdformelem_time:first-of-type",
  ];
  const toCandidates = [
    '[id="1173_to"]',
    '[name="1173[to]"]',
    "#row_ZEIT input.stdformelem_time:nth-of-type(2)",
  ];
...
async function findProjectClickable(formFrame) {
  ...
  for (const sel of selectors) {
    ...
    if (!h) continue;
    const txt = await formFrame
      .evaluate((el) => el.textContent?.trim() || "", h)
      .catch(() => "");
  ...
  for (const x of xp) {
    const h = await waitForXPath(formFrame, x, { timeout: 1500 }).catch(
      () => null
    );
    if (h) return h;
  }
...
async function handleProjektAuswahlDialogOnPage(page, mode /*Office|Remote*/) {
  ...
  const applyBtn = await page.$(
    '#aside_navbar_collapse a[aria-label="Übernehmen"]'
  );
  if (applyBtn) {
